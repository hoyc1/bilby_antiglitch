...
class TestAgreementWithStandardGravitationalWaveTransient(BaseTest):
    ...
    def test_glitch_calculate_snrs_matches_standard_gw_for_unit_signal(self):
        glitch_likelihood = self._base_glitch_likelihood()
        ...
        glitch_snrs = glitch_likelihood.calculate_snrs(
        ...
        assert glitch_snrs.d_inner_h == standard_snrs.d_inner_h
        assert glitch_snrs.optimal_snr_squared == standard_snrs.optimal_snr_squared
        assert glitch_snrs.complex_matched_filter_snr == pytest.approx(
            standard_snrs.complex_matched_filter_snr
        )

    def test_glitch_compute_log_likelihood_matches_standard_gw_for_unit_signal(
        self
    ):
        glitch_likelihood = self._base_glitch_likelihood()
        standard_likelihood = self._base_standard_gw_likelihood()
        ...
        glitch_total = self._network_glitch_snrs(
            glitch_likelihood, unit_signal, parameters={}
        )
        standard_total = self._network_standard_snrs(
            standard_likelihood, unit_signal, parameters={}
        )

        glitch_log_l = glitch_likelihood.compute_log_likelihood_from_snrs(
            glitch_total,
            parameters={}
        )
        ...
        assert glitch_total.d_inner_h == standard_total.d_inner_h
        assert glitch_total.optimal_snr_squared == (
            standard_total.optimal_snr_squared
        )
        assert glitch_log_l == pytest.approx(standard_log_l)

    def test_glitch_log_likelihood_ratio_matches_standard_gw_when_glitch_is_unity(
        self
    ):
        glitch_likelihood = self._base_glitch_likelihood()
        standard_likelihood = self._base_standard_gw_likelihood()

        glitch_likelihood._glitch_model = unit_glitch_model
        glitch_likelihood._glitch_params = ["A", "f", "phi", "tc", "gamma"]
        ...
        glitch_log_l = glitch_likelihood.log_likelihood_ratio(params)
        standard_total = self._network_standard_snrs(
            standard_likelihood,
            np.ones(4, dtype=complex),
            parameters={}
    ...
    def test_glitch_log_likelihood_ratio_matches_standard_gw_for_recalibrated_unit_signal(
        self
    ):
        glitch_likelihood = self._base_glitch_likelihood()
        standard_likelihood = self._base_standard_gw_likelihood()

        glitch_likelihood.calibration_draws = {
            "H1": np.array([[2., 2.]]),
            "L1": np.array([[2., 2.]]),
        }
        standard_likelihood.calibration_draws = {
            "H1": np.array([[2., 2.]]),
            "L1": np.array([[2., 2.]]),
        }
        ...
        glitch_log_l = glitch_likelihood.log_likelihood_ratio(
            {
                "A": 1.,
                "f": 30.,
                "phi": 0.,
